classdef FaultDiagnosisApp < matlab.apps.AppBase
    ...
    methods (Access = public)
        ...
        function precomputeResults(app)
            ...
            % Demo sequence: healthy -> gear_wear sev1/sev2/sev3, all from
            % held-out family 9 (split_family_holdout == "test", never
            % used in training). Severity-1 is the known-hard case --
            % verified separately that this exact file is misclassified
            % as "imbalance" using the model's own pre-computed features,
            % so this arc is a real, pre-confirmed example, not a
            % cherry-picked success case.
            seq = struct( ...
                'fileName', { ...
                ...
                'condition', {'healthy','gear_wear','gear_wear','gear_wear'}, ...
                'severity', {0, 1, 2, 3}, ...
                'jointId', {1, 1, 1, 1});
